...
class BrazilFetcher(base.RiverDataFetcher):
    ...
    def _download_data(
        ...
        try:
            response = s.get(
                self.BASE_URL, params=params, headers=utils.DEFAULT_HEADERS
            )
            ...
            with zipfile.ZipFile(io.BytesIO(response.content)) as zf:
                ...
                if not inner_zip_name:
                    logger.warning(
                        f"Could not find inner zip for site {self.site_id}, variable {variable}"
                    )
                    return None
            ...
            logger.error(
                f"Error downloading from Hidroweb for site {self.site_id}: {e}"
            )
            return None
    ...
    def _parse_data(
        ...
        try:
            ...
            # Pivot longer
            df_long = pd.melt(
                df, id_vars=id_cols, var_name="day_type", value_name="Value"
            )
